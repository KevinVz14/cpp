...
float conversionTemp(float C, float& , float&);
float conversionTemp(float C, float& F , float& K){
...
int main(){
	...
	int opc =0, Base = 0, Altura = 0, Area = 0; 
	...
	switch (opc){
		case 1:{
			...
			//SALIDA
			cout<<" El area del triangulo es: "<<AreaTriangulo(Base,Altura,Area);
			break;
		}
		case 2: {
			//CAPTURA DE DATOS
			...
			cout<<endl;
			//SALIDA
			cout<<"La longitud de la circunferencia es: " <<perimetroCir(r, Pi, L);	
			break;
		}
		case 3: {
			//CAPTURA DE DATOS
